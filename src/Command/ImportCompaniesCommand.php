...
class ImportCompaniesCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        while (true) {
            $companiesData = $legacyDb->fetchAllAssociative("SELECT * FROM companies WHERE created_at >= '2026-04-23 00:00:00' LIMIT $limit OFFSET $offset");
            ...
            foreach ($companiesData as $row) {
                ...
                if (method_exists($company, 'setCity')) {
                    $company->setCity(mb_substr($row['location'] ?? 'Grajewo', 0, 255));
                }
